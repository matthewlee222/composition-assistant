...
PROBLEMS = {
    ...
    "score_function_accurate": ["def score_function_accurate", "def score_function_final"],
}
    ...
    for name, (start, end) in PROBLEMS.items():
        ...
        for checker in CHECKERS:
            ...
            for comment in checker.comments():
                comments.append(Comment(comment.line_num + initial_line_number - 1, comment.comment, comment.fields))
...
class UnnecessaryBooleanComparison(Checker):
    ...
    def visit_Compare(self, node):
        if len(node.ops) == 1:
            ...
            if isinstance(op, (ast.Eq, ast.Is, ast.NotEq, ast.IsNot)) and isinstance(right, ast.NameConstant):
                is_truthy = isinstance(op, (ast.NotEq, ast.IsNot)) != (right.value is True)
                full_line = self.code.split("\n")[node.lineno - 1].strip()
                if isinstance(op, ast.Eq):
                    ...
                    op = "is not"
                true_target = ("not `None`" if is_truthy else "`None`") \
                    if right.value is None \
                    else "`True`" if is_truthy else "`False`"
...
class LongVariableNameChecker(Checker):
    ...
    def visit_Name(self, node):
        name, context = node.id, node.ctx
        if isinstance(node.ctx, ast.Store) and (max(len(part) for part in name.split("_")) > 10 or len(name) > 25):
            if name not in self.variables:
                self.variables[name] = self.Variable()
            var = self.variables[name]
            var.line_num = min(var.line_num, node.lineno)
...
class UnnecessaryRangeArgumentChecker(Checker):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(func, ast.Name) and func.id == "range" and \
                args and isinstance(args[0], ast.Num) and args[0].n == 0:
            self._comments.append(Comment(
                node.lineno,
...
class CamelCaseVariableNamingChecker(Checker):
    ...
    def visit_Name(self, node):
        name, context = node.id, node.ctx
        if isinstance(node.ctx, ast.Store) and name != name.upper() and name != name.lower():
            if name not in self.variables:
                self.variables[name] = self.Variable()
            var = self.variables[name]
            var.line_num = min(var.line_num, node.lineno)
